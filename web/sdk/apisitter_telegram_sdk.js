...
// POSSIBLE ERRORS IN CALLBACK
// null = No error;
// 0 = Input error
// 1 = Generic error send process;
// 2 = Error in API (request code != 200);
// 3 = Error in response reading buffer;
// 4 = Method request unsupported (input)



// async function ClientTelegram() {
//     if (!(this instanceof ClientTelegram)) {
//         return new ClientTelegram();
//     }
//     this.idClientTelegram = null;
//     this.tokenClientTelegram = null;
//     this.socket = null;
// }
...
function ApiSitter() {
    ...
    // Invia il codice al numero di telefono
    // apiSitterEmail: l'email con cui si accede alla dashboard di apiSitter
    // apiSitterToken: il token di apisitter che si trova in settings di apisitter dashboard
    // phone: è il numero di telefono completo (00393343025107) uindi con prefisso internazionale
    // dialCode: è il solo prefisso internazionale (0039)
    // callback(
    //      error,      stringa di errore, null se non ci sono errori
            ...
            xhr.addEventListener("readystatechange", function () {
                if (this.readyState === 4) {
                    if(this.status == 0) {
                        ...
                        return callback(null, this.status, this.responseText);
                    }else{
                        var data = null;
                        try{
    ...
    // Chiama l'API
    // requestType: String "GET" or "POST" type
    // apiName: il nome dell'api che si vuole eseguire
    // parameter:è l'oggetto contenenti i paramtri dell'API (se è "GET" sara null)
    // callback(
    //      error,      stringa di errore, null se non ci sono errori
    //      result,     200, 400, 303 codice di errore della chiamata
    //      body        l'oggetto/stringa di risposta
    ...
    this.callApi = function(requestType, apiName, parameter, callback, options){
        try{
            ...
            xhr.ontimeout = function(data) {
                return callback("GENERIC_ERROR_API", null, "Call timeout error");
            }
    ...
    // Apre il socket e ritona
    // callback(event, data)
    // event: "onConnect", "onUpdate", "onDisconnect", "onError"
    // data: solitamente solo in "onUpdate"
    this.openUpdatesListener = function(callback){
        try {
            var refer = this;
            this.callApi("GET", "getConfigurationWebSocket", null, function(error, result, data){
                if(error){
                    ...
                    refer.socket.on('connect', function(){
                        var auth = {
                            id: refer.idClientTelegram,
    ...
    this.closeUpdatesListener = function(){
